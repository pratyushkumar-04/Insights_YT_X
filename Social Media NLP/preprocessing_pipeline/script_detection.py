...
script_detection.py
-------------------
Identify writing systems (scripts) present in a text.
"""
...
def identify_script(char: str):
    """Return script name for a single char, or None for Common/Inherited."""
    cp = ord(char)
...
def analyze_text(text: str):
    """
    Return structured script analysis:
      - 'scripts': Counter {script_name: count}
      - 'primary': most frequent script, or None
      - 'mixed':   True if 2+ distinct scripts present
      - 'per_char': list of (char, script_or_None)
    """
    counts = Counter()
    per_char = []
...
def format_report(text: str) -> str:
    ...
    if not counts:
        lines.append("No identifiable script found.")
        return "\n".join(lines)
